...
interface ApiKeyModalProps {
  ...
  onSave: (newKey: string) => void; // Function Setter for the current provider's key (setUserApiKey)
  // --- RE-ADD prop for current key value ---
...
export function ApiKeyModal({
  ...
  const {
    ...
    selectedProvider, // <-- Get selected provider
  } = usePrompt();

  // Local state for the input field only
  const [apiKeyInput, setApiKeyInput] = useState('');

  // Sync input with the passed prop when modal opens or prop changes
  useEffect(() => {
    if (isOpen) {
      setApiKeyInput(currentApiKeyProp || ''); // Use prop or default to empty
    }
  ...
  // --- Determine placeholder based on provider ---
    ...
    >
      ...
      >
        ...
        <h2 className="text-xl font-semibold text-gray-800 mb-4">
          {' '}
          Enter {selectedProvider.toUpperCase()} API Key (User Mode){' '}
        </h2>
        {/* Security Warning */}
        <div className="bg-yellow-50 border border-yellow-300 text-yellow-800 text-sm p-3 rounded mb-4">
          ...
          <ul className="list-disc list-inside mt-1">
            <li>
              Your API key will be stored temporarily in your browser's memory{' '}
              <strong className="underline">for this session only</strong>.
            </li>
            <li>
              It will be sent directly from your browser to the AI provider
              (e.g., OpenAI).
            </li>
            <li>
              It will <strong className="underline">never</strong> be sent to or
              stored on our servers when using this mode.
            </li>
            <li>
              Be cautious about entering secret keys into any web application.
              Close the browser tab/window to clear the key from memory.
            </li>
          </ul>
        </div>
        ...
        <div className="text-xs min-h-[18px] mb-4 pl-1">
          ...
          {isInvalid && apiKeyValidationError && (
            <span className="text-red-600">❌{apiKeyValidationError}</span>
          )}
          {isValid && (
