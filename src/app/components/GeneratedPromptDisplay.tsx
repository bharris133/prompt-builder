...
export function GeneratedPromptDisplay() {
    ...
    const handleCopyGenerated = () => { if (!generatedPrompt.trim() || copyStatus) return; navigator.clipboard.writeText(generatedPrompt).then(() => setCopyStatus('copied')).catch(err => { console.error(err); alert('Copy failed'); }); };
        ...
        <section className="p-6 pt-4 border-b border-gray-200 flex-shrink-0">
                ...
                <div className="flex items-center space-x-2">
                 ...
                 {isOpen && (
                     <div className='relative flex items-center'>
                        <span className={`text-xs text-green-600 mr-2 transition-opacity duration-300 ${copyStatus === 'copied' ? 'opacity-100' : 'opacity-0'}`}> Copied! </span>
                        <button
            ...
            {/* Collapsible Content Area Wrapper */}
             <div className={`transition-[max-height] duration-300 ease-in-out overflow-hidden ${ isOpen ? 'max-h-[500px] mt-1' : 'max-h-0' }`}>
                 {/* Content */}
                 <div className={`bg-gray-800 text-white p-4 rounded shadow relative group mb-1`}>
