...
export function PromptCanvas() {
    ...
    return (
        <section className="p-6 pt-6 border-b border-gray-200 flex-shrink-0">
            ...
            <div className={`transition-[max-height] duration-300 ease-in-out overflow-hidden ${ isOpen ? 'max-h-[1500px]' : 'max-h-0' }`}>
                 <div className={`bg-white p-4 rounded shadow min-h-[200px] mt-2`}>
                    <SortableContext items={componentIds} strategy={verticalListSortingStrategy} >
                        {components.length === 0 ? (
                            <p className="text-gray-500 text-center py-10"> Add components or load a saved prompt. </p>
                        ) : (
                            components.map((component) => (
                                <SortablePromptComponent
                                    key={component.id} component={component}
                                    onContentSave={handleContentSave} onDelete={handleDeleteComponent}
                                />
                            ))
                        )}
                    </SortableContext>
